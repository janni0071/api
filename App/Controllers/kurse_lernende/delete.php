...
if (isset($params['id']) && ctype_digit($params['id'])) {
    ...
    try {
        // Delete the kurs_lernende by ID
        $query = 'DELETE FROM tbl_kurse_lernende WHERE id_kurs_lernende = :id_kurs_lernende';

        $stmt = $db->query($query);
        $stmt->execute([':id_kurs_lernende' => $id]);
        ...
        if ($stmt->rowCount() > 0) {
            ...
            Response::json([
                'status' => 'success',
                'message' => 'Kurs_lernende deleted successfully!'
            ], Response::OK);
        } else {
        ...
        Response::json([
            'status' => 'error',
            'message' => 'An error occurred while deleting the kurs_lernende',
